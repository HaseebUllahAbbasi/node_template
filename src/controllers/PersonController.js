...
exports.getAllPersons = catchAsyncErrors(async (req, res) => {
...
exports.deletePerson = catchAsyncErrors(async (req, res) => {
	const { name } = req.params;
	const Persons = await PersonSchema.find();

	const personByName = Persons.filter(
	...
	if (personByName.length == 0) {
		...
		await PersonSchema.deleteOne({ _id: personByName[0]._id });
		res.status(200).json({
			success: true,
		});
	}
...
exports.addPerson = catchAsyncErrors(async (req, res) => {
...
exports.findPersonById = catchAsyncErrors(async (req, res) => {
	const { id } = req.params;
	try {
...
exports.findPersonByName = catchAsyncErrors(async (req, res) => {
	const { name } = req.params;
	const Persons = await PersonSchema.find();
	const personByName = Persons.filter(
		(person) => person.name.toLocaleLowerCase() == name.toLocaleLowerCase()
...
exports.findPersonByCity = catchAsyncErrors(async (req, res) => {
	const { city } = req.params;
	const Persons = await PersonSchema.find();
	const personByName = Persons.filter(
		(person) => person.city.toLocaleLowerCase() == city.toLocaleLowerCase()
	);
	if (personByName.length == 0) {
		res.status(200).json({
			success: false,
